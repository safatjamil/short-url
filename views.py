...
def create_redirection_map(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            object_ = models.RedirectMap(org_id=decr_id, 
                                         org_alias=org_alias, 
...
def delete_redirection_map(request, rl_id):
    if ("signed_in" not in request.session or 
...
def main_redirection(request, short_code):
    ...
    org_alias = short_code[0]
    randcode = short_code[1]
    get_rule = models.RedirectMap.objects.filter(org_alias=org_alias, randcode=randcode).values()
    if not get_rule:
        return HttpResponse("<html><head><title>Error</title></head><body><p>Something went wrong. We can not find a redirection rule for this url.<p></body></html>")
